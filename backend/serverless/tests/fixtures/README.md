...
`test/add-coverage` 브랜치는 테스트 커버리지와 AWS 통합 점검 자료를 보여주는 브랜치입니다. 이 fixture 폴더는 그중 백엔드 로컬 회귀 테스트의 기준 자료를 설명합니다.

AWS 통합 테스트는 아래 위치를 봅니다.

```text
tests/aws/
```

IR 평가 보조 코드는 아래 위치를 봅니다.

```text
evaluation/ir/
```

이 fixture 폴더는 비용이 드는 AWS 테스트가 아니라, 로컬에서 프롬프트 구조 회귀를 확인하는 안전장치입니다.
