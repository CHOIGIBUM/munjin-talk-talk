...
from rag_context import retrieve_alias_hints  # noqa: E402
...
def evaluate_case(case: dict[str, Any]) -> dict[str, Any]:
    ...
    alias_names = [item.get("canonical_hint") for item in alias_hints if item.get("canonical_hint")]
    ranked = ranked_bm25_names(text)

    def top_names(k: int) -> set[str]:
        return {row["name"] for row in ranked[:k]}

    top_by_k = {str(k): top_names(k) for k in (1, 3, 5, 10)}
    ...
    alias_gold_hits = sorted(gold & set(alias_names))
    alias_gold_misses = sorted(gold - set(alias_names))
    alias_negative_hits = sorted(negative & set(alias_names))
...
def make_report(summary: dict[str, Any], records: list[dict[str, Any]]) -> str:
    lines = [
        ...
        f"- BM25 recall@1 / @3 / @5 / @10: {pct(summary['bm25_micro_recall']['recall@1'])} / {pct(summary['bm25_micro_recall']['recall@3'])} / {pct(summary['bm25_micro_recall']['recall@5'])} / {pct(summary['bm25_micro_recall']['recall@10'])}",
        f"- case all-hit@5: {pct(summary['bm25_case_all_hit']['all_hit@5'])}",
        f"- top1 case accuracy: {pct(summary['bm25_top1_case_accuracy'])}",
        f"- negative-in-top5 case rate: {pct(summary['bm25_negative_leak_case_rate']['negative_in_top5'])}",
        "",
        "## Interpretation",
        "",
    ]
    ...
    top5_misses = [row for row in records if row["gold_misses"]["5"]]
    negative_leaks = [row for row in records if row["negative_hits"]["5"]]
    alias_misses = [row for row in records if row["alias_gold_misses"]]
    top1_not_gold = [row for row in records if not row["top1_is_gold"]]

    lines.extend(["", "## Top5 Gold Miss Cases", ""])
    lines.extend(case_lines(top5_misses[:30], include_misses=True))
    lines.extend(["", "## Negative Leakage Cases", ""])
    lines.extend(case_lines(negative_leaks[:30], include_negative=True))
    lines.extend(["", "## Alias Miss Samples", ""])
    lines.extend(case_lines(alias_misses[:30], include_alias=True))
    lines.extend(["", "## Top1 Not Gold Samples", ""])
    lines.extend(case_lines(top1_not_gold[:30], include_misses=False))
    return "\n".join(lines) + "\n"
...
def group_table(group_summary: dict[str, Any]) -> list[str]:
    ...
    for group, item in group_summary.items():
        lines.append(
            ...
            f"{pct(item['bm25_recall@5'])} | {pct(item['all_hit@5'])} | {pct(item['negative_in_top5_rate'])} |"
        )
    return lines
...
def interpret(summary: dict[str, Any]) -> list[str]:
    lines = []
    if summary["bm25_micro_recall"]["recall@5"] >= 0.95:
        lines.append("- IR 후보 검색은 train set 내부에서는 대부분의 gold symptom을 top5 안에 올립니다.")
    else:
        lines.append("- IR 후보 검색이 train set 내부에서도 gold symptom을 충분히 올리지 못합니다.")
    if summary["alias_gold_micro_recall"] < summary["bm25_micro_recall"]["recall@5"]:
        lines.append("- alias hint는 BM25보다 보수적으로 작동합니다. 모든 gold를 직접 alias로 잡는 구조는 아니며, RAG 후보 보조 역할에 가깝습니다.")
    if summary["bm25_negative_leak_case_rate"]["negative_in_top5"] > 0.2:
        lines.append("- 부정된 증상도 lexical 후보에는 자주 올라옵니다. 최종 판정은 LLM span type/status와 IR gate가 막아야 합니다.")
    lines.append("- 이 결과는 학습 데이터 자기검사라 일반화 성능으로 보고하면 안 됩니다. 다음 단계는 별도 test set으로 같은 산식을 반복하는 것입니다.")
    return lines


def case_lines(records: list[dict[str, Any]], *, include_misses: bool = False, include_negative: bool = False, include_alias: bool = False) -> list[str]:
    if not records:
        return ["- none"]
    lines = []
    for row in records:
        top5 = ", ".join(item["name"] for item in row["bm25_top10"][:5])
        line = f"- `{row['case_id']}` {row['question_id']} {row['dialect_type']} | text: {row['text']} | gold: {', '.join(row['gold_symptoms'])} | top5: {top5}"
        if include_misses:
            line += f" | missed@5: {', '.join(row['gold_misses']['5'])}"
        if include_negative:
            line += f" | negative: {', '.join(row['negative_symptoms'])} | negative@5: {', '.join(row['negative_hits']['5'])}"
        if include_alias:
