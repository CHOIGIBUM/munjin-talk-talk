...
이 폴더는 `test/service-validation` 브랜치의 테스트 진입점입니다. 일반 로컬 테스트는 백엔드/프론트엔드 각 폴더에 있고, 이 폴더에는 실제 AWS 배포 환경을 확인하는 수동 통합 테스트를 둡니다.

이 브랜치의 목적은 테스트 비율 수치를 제시하는 것이 아니라, 어떤 검증은 로컬에서 반복 실행 가능하고 어떤 검증은 실제 AWS 리소스를 명시적으로 호출해야 하는지 구분하는 것입니다.
