...
이 문서는 문진톡톡에서 생성, 저장, 전달되는 데이터를 저장소별로 분리해 설명하는 보안 설계 문서입니다.

문진톡톡은 음성 문진, STT, LLM 추출, 증상 매칭, 원페이퍼, 환자 안내문을 다룹니다. 따라서 단순히 "DynamoDB를 쓴다" 또는 "S3를 쓴다" 수준으로는 보안 상태를 설명하기 어렵습니다. 어떤 필드가 개인정보인지, 어떤 필드가 건강정보인지, 어떤 데이터가 LLM으로 전달되는지, 어떤 데이터가 저장 대상에서 제외되는지를 필드 단위로 나누어야 합니다.

이 문서의 목적은 다음과 같습니다.

- DynamoDB에 남기는 최소 세션 데이터와 S3에 보관하는 문진 산출물을 분리합니다.
- 음성 원본, 실명, 생년월일, 연락처처럼 저장하지 않는 데이터를 명확히 합니다.
- Macie, S3 Lifecycle, KMS, IAM, CloudWatch 로그 정책이 어느 저장소에 적용되는지 설명합니다.
- 코드와 AWS 운영 설정이 함께 지키는 보안 경계를 문서화합니다.
...
| 증상 slot, alias, safety flag를 도메인팩으로 분리 | reset | `domain_config.py`, `clinical_terms.py`; train-derived `data/domain_packs/respiratory.json`은 제거됨 |
...
제출 환경에서는 접근 코드 로그인과 만료 세션 토큰으로 직원/의료진 화면을 보호합니다. 병원 실서비스 전환 시에는 Lambda IAM role을 리소스 ARN 단위로 더 좁히고, Cognito 또는 병원 SSO 기반 사용자별 계정과 감사 로그를 적용하는 구조가 적절합니다.
...
아래 표는 문진톡톡의 데이터가 어떤 단계에서 생성되고, 현재 제출 기준 코드에서 어디에 저장되는지 설명합니다.
